...
namelist /input/ in_file_name, out_file_name,  field_names,  pout, &
                 ...
                 use_default_missing_value, verbose, adjtopo, akconv
...
integer :: optind, exitcode, iret
character (len=1024) :: optarg
character (len=8) :: copt
...
!--------- initialize namelist variable and then read namelist ---------
      ...
      pout(1:19) = (/1,5,10,20,30,50,70,100,150,200,250,300,400,500,600,700,850,925,1000/)

      do ivar = 1, MAX_FIELD_NAMES
         do i=1,NF90_MAX_NAME; field_names(ivar)(i:i) = ' '; enddo
      enddo
      field_names(1)=trim(hght_name)
      field_names(2)=trim(slp_name)

      optind=0
      do
        call getopts('i:o:p:c:a:',copt,optarg,optind,exitcode)
        if (exitcode.ne.0) exit
        select case (copt)
        case('i')
          in_file_name = trim(optarg)
          out_file_name = in_file_name(1:len_trim(in_file_name)-3)//"_plev.nc"
        case('o')
          out_file_name = trim(optarg)
        case('c')
          read(optarg,*) compress
        case('p')
          pout(:)=0.0
          read(optarg,*,iostat=iret) pout(:)
          if (iret>0) then
            print *, "Error in user provided output pressure levels"
            print *, trim(optarg)
          endif
        case('a')
          read(optarg,*) adjtopo
        case default
          print *, 'invalid option ', optarg(1:1), copt
        end select
      enddo
...
!   --- if any file name has no length then terminate the program ---
     ...
     if (  in_file_name(1:1) == ' ' .or.   &
          out_file_name(1:1) == ' ' )  then
        print *, "Error: no input files given"
        print *, ""
        print *, "Usage:"
        call getarg(0,optarg)
        print *, trim(optarg)//" -i infile [-o outfile] [-p plev1,plev2,plev3,..] " // &
                 "[-c deflate_level]"
        print *, "" 
        print *, " -o  name of output file [optional]." 
        print *, "" 
        print *, " -p  output pressure levels in hPa [optional], if not provided &
                &  default 19 pressure levels will be used" 
        print *, "" 
        print *, " -c netcdf4 deflate level [0-9] [optional] [Default=3]." 
        print *, "" 
        call exit(1)
     endif
  
     if (trim(in_file_name)==trim(out_file_name)) then
      print *, "Error: infile and outfile name can't be same"
      call exit(1)
     endif
...
!    ------- check to see if required fields are in atmos file -------
     ...
     if (verbose > 1) print *, 'done check fields'
...
!--- get the size of the vertical axis ---
     ...
     nlev = nlev-1
     if (verbose > 1) print *, ' done get the size of the vertical axis'
    ...
    !--- get the names of the input horizontal mass axes ---
     ...
     if (verbose > 1) print *, 'get the names of the input horizontal mass axes'
...
!--- set flags for special non-computed fields ---
     ...
     if (verbose > 1) print *, '2. num_field_names=',num_field_names
     do i = 1, num_field_names
        if (verbose > 1) print *, 'field_name=',trim(field_names(i))
...
!    ---- read vert coord variables and set up vert coord ----
     if (verbose > 1) print *, 'read vert coord variables and set up vert coord' 

     istat = NF90_GET_VAR (ncid_in, varid_bk, bk)
     if (istat /= NF90_NOERR) call error_handler ('get_var bk',ncode=istat)
     if (varid_pk > 0) then
        istat = NF90_GET_VAR (ncid_in, varid_pk, pk)
        pk = pk*akconv
        if (istat /= NF90_NOERR) call error_handler ('get_var pk',ncode=istat)
     else
        pk = 0.0
     endif
    
     if (verbose > 1) print *, 'Done read vert coord variables and set up vert coord' 

!-----------------------------------------------------------------------
!----------------- output file header info -----------------------------
     if (compress>0) then
      istat = NF90_CREATE (trim(out_file_name), ior(NF90_CLOBBER,NF90_NETCDF4), ncid_out, blksz)
     else
   ...
   ! copy global attributes
     ...
     if (istat /= NF90_NOERR) call error_handler (ncode=istat)
     if (verbose > 1) print *, "----------------- output file header info -----------------------------"
...
!    -------- vertical axis (convert to mb) -------
   ...
   ! define new output pressure dimension and variable
     ...
     if (istat /= NF90_NOERR) call error_handler (ncode=istat)
     if(compress>0) then
      istat=nf90_def_var_deflate(ncid_out,varid_pout,1,1,compress)
      if (istat /= NF90_NOERR) call error_handler (ncode=istat)
     endif
...
!---------------- read/write time level --------------
          ...
          if (istat /= NF90_NOERR) call error_handler ('putting time coord value', &
                                                       ncode=istat)
          if (verbose>0) print *, "Done read/write time level"
!-----------------------------------------------------------------------
!-------------- interpolation setup ------------------------------------
     ...
     !---- surface pressure ----
          ...
          if (verbose>0) print *, "Done Reading ps"
...
!     ---- interpolation initialization for mass fields ------
     ...
     if (verbose>0) print *, "Done interpolation initialization for mass fields"

!     ---- interpolation of mass fields ----

     do i = 1, num_field_names
        if ( Fields(i)%skip   ) then
          if(verbose>1) then
            print *, 'Skipping field =', trim(Fields(i)%name)
            cycle
          endif
        endif
...
!#######################################################################
 ...
 real :: tmp(size(values,1),size(values,2),size(values,3))

 integer :: ndim, nd, begin(4), k
   ...
   select case (ndim)
      ...
      case(3)
          ...
          do k=1,size(values,3)
            tmp(:,:,size(values,3)+1-k) = values(:,:,k)
          enddo
          values=tmp
      case default
